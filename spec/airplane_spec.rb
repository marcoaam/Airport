...
describe Airplane do
	...
	it 'can take off' do
		...
		expect(airport).to receive(:allow_to_take_of?).and_return(true)
		plane.take_of_from(airport)
	end

	it 'is flying after take of' do
		allow(airport).to receive(:release).with(landed_plane)
		allow(airport).to receive(:allow_to_take_of?).and_return(true)
	...
	it 'it can not take of if the airport doesnt allow to' do
		allow(airport).to receive(:allow_to_take_of?).and_return(false)
	...
	it 'can land' do
		...
		expect(airport).to receive(:allow_to_land?).and_return(true)
		plane.land_to(airport)
	end

	it 'can not land if the airport doesnt allow to' do
		allow(airport).to receive(:receive).with(plane)
		expect(airport).to receive(:allow_to_land?).and_return(false)
		plane.land_to(airport)
		expect(plane.status).to eq 'flying'
	end
